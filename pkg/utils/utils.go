...
// SetNonOCPDefaultComponents returns true if changes are made
func SetNonOCPDefaultComponents(m *backplanev1.MultiClusterEngine) bool {
	updated := false
	for _, c := range nonOCPComponents {
		if !m.ComponentPresent(c) {
			m.Enable(c)
			updated = true
		}
